 ...
 *        surface reaction for the form proposed by Zhluktov and Abe.
 */
...
class GSIRateLawZAAdsorption : public GSIRateLaw
{
public:
    GSIRateLawZAAdsorption(ARGS args)
        : GSIRateLaw(args),
          ...
          idx_site(1)
    {
        assert(args.s_node_rate_law.tag() == "ZAadsorption");
...
//==============================================================================
        ...
        const Eigen::VectorXd& v_rhoi, const Eigen::VectorXd& v_Tsurf) const
    {
        ...
        return stick_coef_corr * thermal_speed /
            (4 * m_n_sites) * exp(m_beta * log(Tsurf)) * exp(-m_T_act / Tsurf);
    }
...
ObjectProvider<
    GSIRateLawZAAdsorption, GSIRateLaw>
    gsi_rate_law_ZAadsorption("ZAadsorption");
