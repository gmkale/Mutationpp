...
class GSIRateLawArrhenius : public GSIRateLaw
{
public:
    GSIRateLawArrhenius(ARGS args)
        : GSIRateLaw(args),
          ...
          pos_site_r(1)
    {
        assert(args.s_node_rate_law.tag() == "arrhenius");
        ...
        args.s_node_rate_law.getAttribute( "beta", m_beta,
            "The exponential temperature modifier should be provided.");
...
//==============================================================================
	...
	return m_pre_exp*exp(m_beta * log(Tsurf))*exp(-m_T_act / Tsurf); 
    }
...
ObjectProvider<
    GSIRateLawArrhenius, GSIRateLaw>
    gsi_rate_law_arrhenius("arrhenius");
